...
namespace fob {
    namespace interface {
        ...
        EndGame::EndGame(const char * path) :
            Interface(path)
        {
        ...
        void EndGame::UnPause() {
        }

        void EndGame::InitGraphics() {
        ...
        void EndGame::Draw() {
            ...
            char s[256];
            sprintf(s, "%d", _score1);
            fob::system::Font p1score("ingame", 1, 1, 1, 1, 100, 42, s, fob::system::Font::ALIGN_RIGHT);
            p1score.Draw();

            sprintf(s, "%d", _score2);
            fob::system::Font p2score("ingame", 1, 1, 1, 1, 100, -22, s, fob::system::Font::ALIGN_RIGHT);
            p2score.Draw();

            SDL_GL_SwapBuffers();
        }

        void EndGame::HandleMessage(fob::system::Message &msg) {
            fob::system::FrontEndObjectManager::Instance().HandleMessage(msg);
        }

        bool EndGame::PauseEnabled() {
            return false;
        }

        void EndGame::Scores(int score1, int score2) {
            _score1 = score1;
            _score2 = score2;
        }
